<template>
  <base-wrapper>
    ...
    >
      ...
      <view v-if="type=='more'">
        <base-button model="base" title="选择" @click="click_select" :isChoose="more_result_list"/>
      </view>
    </base-wrapper>
  </base-wrapper>
</template>
...
onMounted(() => {
  ...
  if (select_result != null && select_result != '' && select_result != "null") {
    ...
    if (typeof s === 'object' && s !== null && !Array.isArray(s)) {
      ...
      console.log(more_result_list.value,"888881")
    } else {
      // 是列表直接赋值
      more_result_list.value = s
      console.log(more_result_list.value, "888882")
    }
  } else {
    more_result_list.value = []
    console.log(more_result_list.value, "888883")
  }
})
