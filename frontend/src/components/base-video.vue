<template>
    <view>
        ...
        <view v-if="resources.length > 0" class="video-list">
            <video :src="resources[0].url" controls class="video-item"></video>
            <view @click="deleteVideo" class="delete_but">X</view>
        </view>
    </view>
</template>
            ...
            success: (res) => {
                ...
                if (res.statusCode === 200) {
                    ...
                    if (data.code === 0) {
                        ...
                        // fromdatasValue.value = resources.value;
                      resources.value.push(data.data.url);
                      fromdatasValue.value = resources.value.join(',');
                        resolve(data);
                    } else {
                        reject(data.message);
                    }
