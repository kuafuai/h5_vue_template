<template>
  <base-wrapper>
    ...
    <base-layout class="m-t-20 p-t-20" display="flex" direction="c">
      <base-search firstSearchData="userName" :searchData="base_search" firstSearchPlaceholder="请输入要搜索的人员名称"
        :showMoreSearch="false" @refreshTableData="search_click"></base-search>
  ...
  <uni-popup ref="submitPopup" borderRadius="10px 10px 10px 10px" background-color="#fff">
    <view class="m-10 w-full">
      <el-radio-group class="m-20" v-model="submitCheckForm.assigner">
        <el-radio v-for="(item, index ) in submitPersons" :key="index" :value="item.value"> {{ item.label }}
        </el-radio>
      </el-radio-group>
